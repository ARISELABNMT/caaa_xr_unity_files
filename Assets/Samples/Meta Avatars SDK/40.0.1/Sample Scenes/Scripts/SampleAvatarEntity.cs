...
#pragma warning restore CS0414
    ...
    // This is a solo scene with a single camera (CenterEyeAnchor) — there is no separate observer
    // camera, so even ThirdPerson-only view would still render to that same camera. Fully hide the
    // avatar's visible mesh (None) so only the OVR hand-tracking mesh is seen in-headset, while
    // keeping the entity/skeleton alive for other scripts (e.g. risk calculation) to read joint data.
    protected override void ConfigureEntity()
    {
        base.ConfigureEntity();
